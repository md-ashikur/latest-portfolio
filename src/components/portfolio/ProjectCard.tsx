...
const ProjectCard = ({ project }: ProjectCardProps) => {
  return (
    <Link href={`/portfolio/${project.id}`}>
      <div className="project-card group relative rounded-3xl bg-white overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-700 cursor-pointer h-full flex flex-col">
        ...
        <div className="absolute top-4 right-4 z-10 flex flex-col gap-2">
          <span className="px-3 py-1 text-xs font-bold rounded-full bg-white/95 backdrop-blur-sm text-gray-700 shadow-lg border border-gray-100">
            {project.category}
          </span>
          {project.featured && (
        ...
        <div className="p-6 flex-1 flex flex-col">
          <div className="flex items-start justify-between gap-3 mb-3">
            ...
            <div className="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center group-hover:bg-primary group-hover:scale-110 transition-all duration-300 shrink-0">
              <HiArrowUpRight className="w-4 h-4 text-gray-600 group-hover:text-black group-hover:rotate-45 transition-transform duration-300" />
            </div>
          </div>

          <p className="text-xs text-gray-400 font-medium mb-3">{project.role}</p>
          ...
          <div className="flex flex-wrap gap-2 mt-auto">
            {project.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-gray-50 text-gray-600 border border-gray-100 group-hover:border-primary/30 group-hover:text-primary transition-all duration-300"
              >
                {tag}
              </span>
            ))}
            {project.tags.length > 3 && (
              <span className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-gray-50 text-gray-400 border border-gray-100">
                +{project.tags.length - 3}
              </span>
            )}
          </div>
